...
Output ONLY valid JSON:
{
  ...
  "npc_updates": [
    {
      ...
      "appearance": "concrete physical/visual description — skin tone, hair (state it explicitly: color and style, or bald/shaved), build and body proportions, face, apparent age, clothing, distinguishing features (only if newly described)",
  ...
  "hero_tells": [
    { "id": "ONLY when this is another sighting of a pattern listed under KNOWN HERO TELLS — that tell's exact id; omit for a new one", "text": "a VISIBLE habit, manner, or way of the HERO that a character present could notice, stated as the observable pattern ('digs out his pipe and walks off when a conversation turns on him'; 'jokes the moment things get serious'; 'pays double without being asked'; 'never draws first')", "kind": "manner|habit|principle|intimate", "witnesses": ["exact names of the characters who SAW it this turn"], "public": "true ONLY when this sighting happened before uninvolved bystanders (a tavern, a market, a road with travelers) — omit otherwise; never for an intimate tell", "voiced": "true ONLY when a character SAID this pattern about the hero aloud this turn (a tease, a guess, a serious question) — then also give voicedBy; omit otherwise", "voicedBy": "exact name of the character who said it — only with voiced", "sighted": "with voiced: true ONLY if the hero ALSO visibly did the thing this turn (a remark alone is not a sighting)" }
  ],
            ...
            dispatch({
                ...
                payload: {
                    ...
                    profile: {
                        ...
                        danger: locationProfile.danger,
                        region: locationProfile.region,
                        // The place card (2026-09-16): typed + clamped by
                        // normalizeLocationRecord at the reducer boundary.
                        signature: locationProfile.signature,
                        lastState: locationProfile.last_state ?? locationProfile.lastState,
                    },
