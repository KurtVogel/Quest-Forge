...
- **Progression:** `progression.js` owns XP thresholds (D&D 5e-style per-level increments: 300 XP for level 1 → 2, 600 for level 2 → 3, etc.), the D&D max-level cap (20), fixed average HP-on-level-up (`floor(hitDie / 2) + 1 + CON`, minimum 1), and feature unlocks. Fighter is the only class with a coded combat bonus (`getLevelBonus`, capped at +3) and Extra Attack at L5.
