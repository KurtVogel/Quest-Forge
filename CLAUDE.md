...
- **Combat batch safeguard + mobile panel:** if the DM returns only enemy/companion rolls after a declared player attack, `rollResolver.js` restores the omitted attack(s) when the target is unambiguous (including active Action Surge), or blocks the exchange without rolling enemies/advancing the round when it is ambiguous. Every player-turn batch is canonicalized into player → companion → enemy order, and each enemy may attack at most once across the whole recursive exchange; Action Surge never grants enemies extra responses. At phone widths the Combat panel defaults to a compact round/status/live-foe HP summary with full details on demand; desktop stays expanded.
