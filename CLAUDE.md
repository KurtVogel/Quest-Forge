...
- **Out-of-combat check discipline:** dice are exceptional, not the default response to roleplay. A check requires genuine uncertainty, active opposition/pressure, and an interesting failure consequence; routine competence and approaches that already solve the obstacle succeed without a roll. Clever positioning earns automatic success, advantage, or a lower DC. The solo-play ladder is 8 easy under pressure / 10 standard / 12 meaningful / 15 strong opposition / 18+ exceptional—never default DC 15. Failed social checks control the NPC's response, not the player's authored words, confidence, feelings, or delivery.
