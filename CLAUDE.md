...
- **Progression:** `progression.js` owns XP thresholds (D&D 5e-style per-level increments: 300 XP for level 1 → 2, 600 for level 2 → 3, etc.), rolled HP-on-level-up, and feature unlocks. Fighter is the only class with a coded combat bonus (`getLevelBonus`, capped at +3) and Extra Attack at L5.
