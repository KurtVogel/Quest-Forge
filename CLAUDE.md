...
- **Combat batch safeguard + mobile panel:** after a declared player attack, `rollResolver.js` requires a resolvable player attack before any enemy/companion roll. It restores omitted attacks, repairs malformed attacks missing `skill`/target/DC, rejects damage-only placeholders, and restores every expected Attack action during active Action Surge when the target is unambiguous; otherwise it blocks the exchange without rolling enemies/advancing the round. Every player-turn batch is canonicalized into player → companion → enemy order, and each enemy may attack at most once across the whole recursive exchange. At phone widths the Combat panel defaults to a compact round/status/live-foe HP summary with full details on demand; desktop stays expanded.
