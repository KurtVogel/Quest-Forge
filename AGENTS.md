...
- **Out-of-combat check discipline:** dice are exceptional, not the default response to roleplay. A check requires genuine uncertainty, active opposition/pressure, and an interesting failure consequence; routine competence and approaches that already solve the obstacle succeed without a roll. Clever positioning earns automatic success, advantage, or a lower DC. Outside-combat `requested_rolls` carry `advantage`/`disadvantage`; `rollResolver.js` rolls two d20s, keeps high/low, combines condition effects, and shows the result. The DM adjudicates whether fiction warrants it; the engine owns execution. The solo-play ladder is 8 easy under pressure / 10 standard / 12 meaningful / 15 strong opposition / 18+ exceptional—never default DC 15. Failed social checks control NPC response, not the player's authored delivery. `outOfCombatRollPolicy.js` specifically rejects Persuasion/Deception-style belief/innocence checks when the player explicitly says they are answering truthfully and asks for no concrete concession; ChatPanel hides the invalid setup and requests a no-roll NPC response. NPCs may still doubt/probe from motives and evidence, while release/access/aid/risk or another concrete concession may warrant a roll.
