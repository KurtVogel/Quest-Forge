...
- **Combat:** `START_COMBAT` builds the initiative order; enemy HP/condition are tracked client-side; `END_COMBAT` has a client-side XP fallback if the DM forgets `exp_awarded`. Solo level-1 encounters are auto-tamed (`balanceSoloLevelOneEncounter`).
