...
- [ ] **P1** (rules-math, 2026-07-13): `isProficientWithWeapon` (`engine/rules.js:117-138`) — silently subtracts proficiency bonus from attack rolls and drives the DM-prompt/Inventory "not proficient" flags — has zero test coverage anywhere; every `getWeaponAttackBonus` test uses a proficient Fighter, so the `return false` penalty branch is never exercised end-to-end.
- [ ] **P1** (rules-math, 2026-07-13): `getMaxHitPoints`'s multi-level branch (`engine/rules.js:372-381`) is dead in production (only ever called with `level=1`, `engine/characterUtils.js:184`) because `progression.js` independently reimplements the same per-level formula for actual leveling — but with a `Math.max(1, ...)` floor `getMaxHitPoints` lacks. Untested; a future `level>1` caller with very low CON could produce negative HP growth.
