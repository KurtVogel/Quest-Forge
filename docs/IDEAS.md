...
### Hero tells — the world notices the hero's MANNER (habits, tone, principles, intimate knowledge) — status: `shipped` (2026-09-23, all five slices)
Vesa's idea: others register not only what the hero DID but how they are — "Let me guess, he dug out his pipe and walked away?", "He's never serious. Is he afraid of something?" — so the player catches sight of their own patterns; and a lover's knowledge of the hero's body and ways in bed is in the same lane, because the reward is that the companion can tease, joke, or talk seriously with it. Shipped (DECISIONS.md 2026-09-23 ×2): Scribe `hero_tells` sightings → engine-owned pattern (3 different scenes; intimate = one night, partner-only) → witness-only standing block + an engine-timed pointed-remark window (relationship-beat pattern); same day the four follow-ups below shipped too (all `[x]`). **Still open:** a bond moment on the beat's speaker when the remark lands hard (needs the Scribe to grade it — cheap, later); a live playtest watching for over-use and motive-guessing at the Scribe.
- [x] **The character sheet's "How others see you"** — established tells that have been VOICED at least once (`voicedCount > 0`, so the player hears it first and reads it second), with who said it; a player toggle to strike a tell they reject ("that's not me") that sets it dormant. Zero LLM.
- [x] **Tells travel as hearsay** — a non-intimate tell witnessed in public (`witnessed`-style flag on the sighting) joins the traveling-rumor sources so a stranger in the next town says "you're the one who never draws first, aren't you?" — distortion-graded like deeds; intimate tells never travel (secrets never travel).
- [x] **Voiced-by-the-fiction** — the Scribe reports `voiced: true` beside a sighting when a character actually said it, so the cooldown stamp reads the fiction instead of assuming the window landed; the beat's speaker gets a bond moment of kind `confession`/`quarrel` when the remark lands hard.
- [x] **Player-chosen change** — a tell not sighted for 200 messages already fades; add a "you noticed I stopped?" beat: when a faded tell's witness is present, once, they may remark on its ABSENCE.
...
- **Harness:** (a) the plant step asked "what do you want" three times and took the LAST answer as ground truth while the record lane answered from the first — plant once, take the first established answer; (b) the scripted travel lines did not match a job that lives in the inn's cellar — a Flash-driven adaptive "player agent" that reads the state and picks the next line would cover travel/quest turn-in properly.
- ~~**Receipt subject list noise** (cosmetic): place-name variants pile up next to the person in the `📜 From the record for …` line~~ — **retired 2026-09-21:** the receipt is inspector-only now (no chat line), and the subject matcher requires a distinctive token, so role-word and description "people" no longer join it; place-name variants of one record still can (inspector-only, cosmetic).
...
**Unscripted proof run 2026-09-16 (the road-is-a-scene clause):** the prose side works — Gemini said bearing + duration + road in-world on 4 of 4 legs with nothing in the player's line, and both consistency probes held — but the LINKS did not fill (1 of 4 legs detailed, 0 town-to-town edges): the clause splits a journey across two turns, so the DM's end-of-turn `location` names the ROAD (minting "The Millrace Road" / "The Upper Vellmark" as places), the destination town never mints under its own name when the hero walks straight into its tavern, and the Scribe's `travel` field only reports a journey COMPLETED inside one exchange (fired once). Next slice, engine-side: (a) let `travel` report an in-progress journey (`from`, `to`, bearing said on the departure turn) and let the ARRIVAL turn's ADD_TRAVEL_LINK read the previous narration as evidence too; (b) fold a road-/river-named record ("the X Road", "the Upper Y") into the edge between the places it joins instead of minting a node — the same token rule that already refuses scene descriptions; (c) when the DM's location is a sub-place of a NAMED town the premise/turn text mentions, mint the town record as the parent. Until then "Known ways" stays sparse on unscripted play; the prompt clause is worth keeping for the prose alone.
