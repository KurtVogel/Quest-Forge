...
### Hero tells — the world notices the hero's MANNER (habits, tone, principles, intimate knowledge) — status: `shipped, slice 1` (2026-09-23)
Vesa's idea: others register not only what the hero DID but how they are — "Let me guess, he dug out his pipe and walked away?", "He's never serious. Is he afraid of something?" — so the player catches sight of their own patterns; and a lover's knowledge of the hero's body and ways in bed is in the same lane, because the reward is that the companion can tease, joke, or talk seriously with it. Shipped (DECISIONS.md 2026-09-23): Scribe `hero_tells` sightings → engine-owned pattern (3 different scenes; intimate = one night, partner-only) → witness-only standing block + an engine-timed pointed-remark window (relationship-beat pattern). **Later slices, in order of value:**
- **The character sheet's "How others see you"** — established tells that have been VOICED at least once (`voicedCount > 0`, so the player hears it first and reads it second), with who said it; a player toggle to strike a tell they reject ("that's not me") that sets it dormant. Zero LLM.
- **Tells travel as hearsay** — a non-intimate tell witnessed in public (`witnessed`-style flag on the sighting) joins the traveling-rumor sources so a stranger in the next town says "you're the one who never draws first, aren't you?" — distortion-graded like deeds; intimate tells never travel (secrets never travel).
- **Voiced-by-the-fiction** — the Scribe reports `voiced: true` beside a sighting when a character actually said it, so the cooldown stamp reads the fiction instead of assuming the window landed; the beat's speaker gets a bond moment of kind `confession`/`quarrel` when the remark lands hard.
- **Player-chosen change** — a tell not sighted for 200 messages already fades; add a "you noticed I stopped?" beat: when a faded tell's witness is present, once, they may remark on its ABSENCE.
