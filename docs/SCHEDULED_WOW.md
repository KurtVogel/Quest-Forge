...
- [ ] **W1** (exploration-travel, 2026-09-16): The place card — a Scribe-captured `signature` (first-stated, merged like appearance) and `lastState` (current-state replace) on the location record, plus derived residents (`basedIn`), "what happened here" (journal entries stamped at the record), and in-fiction visit counts; rendered on the Places tab and as ONE line beside Current location in the DM prompt — entry 2026-09-16
