...
All 8 lines of the 2026-09-12 scheduled audit (Lap 2, hostile input) cleared, every line ticked
with a fix note. **Ruling: DECISIONS.md 2026-09-13.** **P1s:** (1) own-key gates — `isKnownRace`/
`isKnownClass` (`Object.hasOwn`, string-only, `engine/characterUtils.js`) behind the vault, the
load heal, both display names, and the style/archetype normalizers, so `class: 'constructor'`
is "Unknown class" on import and heals to Fighter with a notice on load instead of a featureless
"Object"; (2) `catalogEntry`/`catalogKeyForName` behind every `ITEM_CATALOG[`/`NAME_TO_KEY[` read,
so an item named "Constructor" mints a plain custom row instead of throwing out of `ADD_ITEM`,
`PURCHASE_ITEM`, the vault, and the load heal (an unloadable campaign). **P2s:** vault identity
text through `cleanTextField`; `toFlag` booleans (catalog rows keep only catalog flags,
applyEvents reuses it, `isShieldItem` is `=== true`); non-catalog `type` lowercased + whitelisted
and `damage` bounded whenever present; `boundHealingNotation` (≤10d12+20, else 2d4+2); descriptive
